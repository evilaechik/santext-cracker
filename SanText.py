...
def SanText_plus_init(prob_matrix_init, word2id_init, sword2id_init, id2word_init, all_words_init, p_init, tokenizer_init):
    global prob_matrix
    global word2id
    global sword2id
    global id2word
    ...
    for word in doc:
        if word in word2id:
            if word in sword2id:
                ...
                sampling_index = np.random.choice(len(sampling_prob), 1, p=sampling_prob)
                new_ids.append(sampling_index[0])
            else:
                ...
                if flip_p <= p:
                    ...
                    new_ids.append(word2id[word])
        else:
            sampling_prob = 1 / len(all_words) * np.ones(len(all_words), )
            sampling_index = np.random.choice(len(sampling_prob), 1, p=sampling_prob)
            new_ids.append(sampling_index[0])

    return " ".join([id2word[i] for i in new_ids])
